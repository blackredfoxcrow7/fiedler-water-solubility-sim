...
def generate_fractal_water_3d(smiles, mol_name, steps=100):
    G_init, w_os = fsim_frac.fsim_base.create_integrated_system([smiles], water_per_atom=3, is_loop=True)
    f_hist, frames_data, delta_f_final = fsim_frac.run_fiedler_fractal_opt_sim(G_init, w_os, steps=steps, alpha_powerlow=2.0)
    ...
    # Active Fractal Water-Water Network (Cyan Lines)
    ww_x, ww_y, ww_z = [], [], []
    for u, v, d in G_final.edges(data=True):
    ...
    fig.add_trace(go.Scatter3d(
        ...
        line=dict(color='cyan', width=4), name='Active Fractal Water Network', visible=True
    ))

    # Solute-Water H-Bonds (Lime Dashed)
    sw_x, sw_y, sw_z = [], [], []
    for u, v, d in G_final.edges(data=True):
        etype = d.get('edge_type')
    ...
    updatemenus = [
        dict(
            ...
            buttons=[
                dict(label="🌐 すべて表示 (Show All)", method="update",
                     args=[{"visible": [True, True, True, True, True]}, {"title": f"Active Fractal Water (All): {mol_name} | Δf: {delta_f_final:.4f}"}]),
                dict(label="🧪 溶質のみ (Solute Only)", method="update",
                     args=[{"visible": [True, True, False, False, False]}, {"title": f"Active Fractal Water (Solute Only): {mol_name} | Δf: {delta_f_final:.4f}"}]),
                dict(label="💧 フラクタル水網のみ (Fractal Water Only)", method="update",
                     args=[{"visible": [False, False, True, True, False]}, {"title": f"Active Fractal Water Network Only: {mol_name} | Δf: {delta_f_final:.4f}"}])
            ],
            pad={"r": 10, "t": 10}, showactive=True, x=0.02, xanchor="left", y=1.12, yanchor="top", font=dict(color="black", size=12)
        )
    ]

    fig.update_layout(
        title=f"Active Fractal Water Network: {mol_name} ({smiles}) | Δf_final: {delta_f_final:.4f}",
        updatemenus=updatemenus,
        scene=dict(xaxis=dict(visible=False), yaxis=dict(visible=False), zaxis=dict(visible=False), bgcolor='rgb(15, 23, 42)'),
        margin=dict(l=0, r=0, b=0, t=80)
    )

    out_html = f"/home/eldenring/waterMain/active_fractal_3d_{mol_name.lower()}.html"
    fig.write_html(out_html)
    print(f"  ➜ 積極的フラクタル 3D HTML 出力完了: {out_html}")
    return out_html

generate_fractal_water_3d("OCC(O)CO", "glycerin")
