...
def create_integrated_system(smiles_list, water_per_atom=2, is_loop=True):
    ...
    for mol_idx, smiles in enumerate(smiles_list):
        ...
        for atom in mol.GetAtoms():
            ...
            # 水素結合の最大許容数（次数制限）を設定
            if sym == 'O':
                max_h_bonds = 2      # 酸素（エーテル、カルボニルなど）は最大 2 本
                num_waters = 2
            elif sym == 'N':
                max_h_bonds = 3      # 窒素は最大 3 本
                num_waters = 3
            elif category in ['polar', 'ion']:
                max_h_bonds = 2
                num_waters = 2
            else:
                max_h_bonds = 1      # 疎水性炭素・水素などは最大 1 本
                num_waters = 1
            ...
            for u, v, d in G.edges(data=True):
                ...
                if etype in ['opt_water', 'dummy_solvation']:
                    ...
                    if u_idx is not None and v_idx is not None:
                        ...
                        # 親水性のダミー水和は距離限界(0.8)を超えた場合のみ切る
                        # それ以外（疎水性ダミーやopt_water）は距離限界またはFiedler寄与が低い場合に切る
...
def visualize_results(f_history, frames_data, allow_long_distance_water=False):
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title=f"Topology Optimization via Fiedler Vector{mode_text} (Solute Degree Limit)",
