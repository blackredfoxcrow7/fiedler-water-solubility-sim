...
def run_fiedler_opt_sim(G_init, w_os, steps=100, allow_long_distance_water=False):
    ...
    for t in range(steps):
        ...
        # --- SIMPLE UNIFIED RULES (No step thresholds t, no arbitrary probabilities) ---
        
        # 1. GEOMETRIC CUT: Cut any non-covalent edge if it stretches too far
        to_remove = []
        for u, v, d in G.edges(data=True):
            etype = d.get('edge_type')
            if etype in ['opt_water', 'dummy_solvation', 'agg']:
                dist = np.linalg.norm(pos[u] - pos[v])
                is_ww = (u in w_os and v in w_os)
                
                # Cut if distance exceeds physical limit (0.8)
                if dist > 0.8 and not (allow_long_distance_water and is_ww):
                    to_remove.append((u, v))
                    
                # Dehydration: Hydrophobic atoms discard water when they start to separate (> 0.35)
                elif etype == 'dummy_solvation':
                    s_node = u if 'S_' in str(u) else v
                    if G.nodes[s_node].get('category') == 'hydrophobic' and dist > 0.35:
                        to_remove.append((u, v))
                        
        G.remove_edges_from(to_remove)
        
        # Update graph indices after geometric cuts
        nodelist = list(G.nodes())
        node_to_idx = {n: i for i, n in enumerate(nodelist)}
        evals, f_val, f_vec = analyze_laplacian_with_vector(G)

        # 2. SPECTRAL EDGE SWAP (Keep connections active, swap least useful for most useful)
        if nx.is_connected(G):
            # Find the active non-covalent edge with the SMALLEST Fiedler difference (least important)
            active_edges = []
            for u, v, d in G.edges(data=True):
                etype = d.get('edge_type')
                if etype in ['opt_water', 'dummy_solvation']:
                    ...
                    if u_idx is not None and v_idx is not None:
                        ...
                        active_edges.append((diff, u, v))
            
            # Cut the single least important edge if its contribution is tiny
            if active_edges:
                active_edges.sort(key=lambda x: x[0]) # smallest diff first
                min_diff, cu, cv = active_edges[0]
                if min_diff < 0.05:
                    G.remove_edge(cu, cv)
            
            # Recompute Fiedler after swap cut
            nodelist = list(G.nodes())
            node_to_idx = {n: i for i, n in enumerate(nodelist)}
            evals, f_val, f_vec = analyze_laplacian_with_vector(G)
            
            # Find the potential edge with the LARGEST Fiedler difference (most important)
            if nx.is_connected(G):
                ...
                for i in range(len(candidate_nodes)):
                    for j in range(i+1, len(candidate_nodes)):
                        ...
                        if not G.has_edge(u, v) and degrees.get(u, 0) < 4 and degrees.get(v, 0) < 4:
                            dist = np.linalg.norm(pos[u] - pos[v])
                            if dist < 0.6:
                ...
                # Add the single best edge
                if potentials:
                    potentials.sort(reverse=True, key=lambda x: x[0]) # largest diff first
                    best_diff, au, av = potentials[0]
                    G.add_edge(au, av, edge_type='opt_water', color='lime')

        # 3. HYDROPHOBIC COLLAPSE (Intermolecular-only aggregation to match original force balance)
        for i, u in enumerate(s_nodes):
            for v in s_nodes[i+1:]:
                if G.nodes[u].get('molecule_id') != G.nodes[v].get('molecule_id') and \
                   G.nodes[u].get('category') == 'hydrophobic' and np.linalg.norm(pos[u] - pos[v]) < 0.20:
                    if not G.has_edge(u, v):
                        G.add_edge(u, v, edge_type='agg', color='orange')

        # spring_layout による座標更新
        pos = nx.spring_layout(G, dim=3, pos=pos, iterations=4, seed=42)
        
        # --- 立体衝突回避 (Scale-Aware Steric Clash Resolution) ---
        for _ in range(6):
            clashes_resolved = True
            for i, u in enumerate(s_nodes):
                for v in s_nodes[i+1:]:
                    diff = pos[u] - pos[v]
                    dist = np.linalg.norm(diff)
                    
                    sym_u = G.nodes[u].get('symbol', 'C')
                    sym_v = G.nodes[v].get('symbol', 'C')
                    min_dist = 0.045 if (sym_u == 'H' or sym_v == 'H') else 0.065
                    
                    if dist < min_dist:
                        clashes_resolved = False
                        if dist < 1e-4:
                            diff = np.random.normal(0, 0.01, 3)
                            dist = np.linalg.norm(diff)
                        push = 0.45 * (min_dist - dist) * (diff / dist)
                        pos[u] += push
                        pos[v] -= push
            if clashes_resolved:
                break
...
def visualize_results(f_history, frames_data, allow_long_distance_water=False):
    ...
    for t, (G_t, pos_t, _) in enumerate(frames_data):
        ...
        plot_data = [
            ...
            go.Scatter3d(x=opt_x, y=opt_y, z=opt_z, mode='lines', line=dict(color='lime', width=2), opacity=0.9),
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title=f"Topology Optimization via Fiedler Vector{mode_text} (Dense Oil Drop)",
...
if __name__ == "__main__":
    smiles_list = ["CC(=O)N[C@@H](C)C(=O)N[C@@H](C)C(=O)N[C@@H](C)C(=O)N[C@@H](C)C(=O)N[C@@H](CCCCN)C(=O)N[C@@H](C)C(=O)N[C@@H](C)C(=O)N[C@@H](C)C(=O)N[C@@H](C)C(=O)N[C@@H](CCCCN)C(=O)N[C@@H](C)C(=O)N[C@@H](C)C(=O)N[C@@H](C)C(=O)N[C@@H](C)C(=O)N[C@@H](CCCCN)C(N)=O"]
    print(smiles_list)
