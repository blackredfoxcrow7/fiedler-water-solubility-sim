...
# --- 3. 衝突回避 (Clash Avoidance Step) ---
def apply_clash_avoidance(G, pos):
    nodes = list(G.nodes())
    for i in range(len(nodes)):
        for j in range(i+1, len(nodes)):
            u, v = nodes[i], nodes[j]
            # 共有結合ペアは除外
            if G.has_edge(u, v) and G.edges[u, v].get('edge_type') in ['covalent', 'covalent_w']:
                continue
                
            sym_u = G.nodes[u].get('symbol', '')
            sym_v = G.nodes[v].get('symbol', '')
            
            # スプリングレイアウトの無次元スケール適応判定値
            ...
            if dist < min_dist:
                ...
                # 両原子を少し引き離す
                pos[u] -= direction * (overlap * 0.5)
                pos[v] += direction * (overlap * 0.5)
    return pos

# --- 4. 最適化志向AIシミュレーター (衝突回避統合版) ---
def run_fiedler_opt_sim(G_init, w_os, steps=100, allow_long_distance_water=False):
    ...
    for t in range(steps):
        ...
        pos = apply_clash_avoidance(G, pos)  # めり込み防止
