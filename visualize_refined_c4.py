...
import fiedler_optimization_sim_refined as fsim
...
def generate_interactive_html(f_history, frames_data, output_path, title):
    print(f"Generating 3D interactive HTML for {title} (Refined Simulator)...")
    frames = []
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title=dict(text=f"{title} Trajectory (Refined Simulator - Full System)", font=dict(color='white', size=18)),
...
if __name__ == "__main__":
    # Diethyl Ether
    G_init, w_os = fsim.create_integrated_system(["CCOCC"], water_per_atom=3, is_loop=True)
    f_history, frames_data = fsim.run_fiedler_opt_sim(G_init, w_os, steps=100, allow_long_distance_water=False)
    generate_interactive_html(f_history, frames_data, "/home/eldenring/waterMain/diethyl_ether_refined_simulation.html", "Diethyl Ether")

    # tert-Butanol
    G_init, w_os = fsim.create_integrated_system(["CC(C)(C)O"], water_per_atom=3, is_loop=True)
    f_history, frames_data = fsim.run_fiedler_opt_sim(G_init, w_os, steps=100, allow_long_distance_water=False)
    generate_interactive_html(f_history, frames_data, "/home/eldenring/waterMain/tert_butanol_refined_simulation.html", "tert-Butanol")
