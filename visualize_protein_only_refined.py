...
"""
visualize_protein_only_refined.py

このスクリプトは、fiedler_optimization_sim_refined.py のシミュレーションをそのまま実行し、
水分子（溶媒）を取り除いた「入力タンパク質（溶質アトム）部分のみ」を抽出して
黒背景に映えるプレミアムカラーで3D可視化します。
"""
...
try:
    ...
    print("Error: fiedler_optimization_sim_refined.py が見つかりません。同じディレクトリに配置してください。")
    sys.exit(1)
...
def extract_and_visualize_protein_only(f_history, frames_data):
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title=dict(
                text="Protein-Only 3D Folding Simulation (Refined Physics)",
    ...
    # HTMLファイルとして保存
    html_filename = "protein_only_simulation_refined.html"
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(" 🧪 Fiedler-driven Topology Optimization Simulation (Protein Only - Refined)")
    print("=" * 60)
    ...
    plot_filename = "fiedler_evolution_refined.png"
